...
class SalaryController extends PublicController {
    ...
    public function commissionList(){
        ...
        if (!$roleCommission) {
            //数据库没有平台上个月待发提成记录
            $roleCommissionList = $this->computeCommission($staffIdList,$roleId);
        }else{
                ...
                foreach ($commissionRule as $c) {
                    foreach ($staffList as $k=>$v) {
                        if (in_array($v['staff_id'],$c['participant'])) {
                            $commission = $roleCommission[0]['commission']*($c['commission']/100);
                            unset($staffList[$k]);
    ...
    //计算提成并写入数据库
    private function computeCommission($staffIdList=array(),$roleId){
        $phase = strtotime(date('Y-m-1',strtotime('-1 month')));
        ...
        if (in_array($roleId,$onlineStore)) {    //平台销量提成 
            ...
            $roleSales   = D('Salary')->sales($startTime,$endTime,$status);  // 上月销量
            if ($roleSales) {
